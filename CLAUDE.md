...
- **Live input masking/formatting (e.g. Guest List's phone number field) can't be done with a plain `TextField` and a custom `Binding`.** The setter can transform the stored value, but the field's on-screen text — backed by a real `UITextField` under the hood — only catches up once the field loses focus, not while it's actively being typed into. Getting the displayed text and cursor position to update together, live, needs a `UIViewRepresentable` wrapping `UITextField` directly, with a `Coordinator`/`UITextFieldDelegate` reacting to `.editingChanged` — see `PhoneNumberTextField` in `GuestListView.swift`.
